...
/*   Updated: 2025/12/08 11:47:24 by pcordero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	output = NULL;
	eol_found = 0;
	while (eol_found == 0 && gnl[fd].buffer != NULL)
	{
		ft_gnl_read(&(gnl[fd]), fd);
		if (gnl[fd].r_bytes < 1)
		{
			free(gnl[fd].buffer);
			gnl[fd].buffer = NULL;
		}
		if (gnl[fd].r_bytes < 0 && output != NULL)
		{
			free(output);
			return (NULL);
		}
		output = ft_gnl_output(&(gnl[fd]), &output, &eol_found);
	}
	return (output);
}
